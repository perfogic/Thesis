...
} from "pages/BitcoinDashboard/@types";
import {
...
export const PendingWithdraws: React.FC<{}> = ({}) => {
  ...
  const previousOutputs = checkpointPreviousData?.transaction.data.output
    ...
    : [];
  const finalOutputs = hasSigningCheckpoint
    ? [...allOutputs, ...previousOutputs]
    : allOutputs;
  ...
  const handleNavigate = (txid: String) => {
    window.open(`https://blockstream.info/testnet/address/${txid}`, "_blank");
  };
  ...
  const headers: TableHeaderProps<TransactionParsedOutput> = {
      ...
      accessor: (_) => (
        <div className={styles.symbols}>
          <div className={styles.symbols_logo}>
            {generateIcon(tokens.oraichain, tokens.bitcoin)}
          </div>
        </div>
      ),
      width: "12%",
    ...
    amount: {
      ...
      sortField: "value",
      accessor: (data) => (
        <span>{toDisplay(BigInt(data.value || 0), 8)} BTC</span>
      ),
    },
  };
  const checkRenderUI = () => {
