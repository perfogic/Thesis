...
export const useGetDepositFee = (checkpointIndex?: number) => {
  const { data } = useQuery(
    ["deposit_fees", checkpointIndex],
    () => getDepositFee(checkpointIndex),
    {
      refetchOnWindowFocus: true,
...
export const useGetWithdrawalFee = (
  ...
  const getWithdrawalFee = async (
    btcAddress: String,
    checkpointIndex?: number
  ): Promise<WithdrawalFeeInterface> => {
    try {
...
export const useGetCheckpointData = (checkpointIndex?: number) => {
  const { data } = useQuery(
    ["bitcoin_checkpoint", checkpointIndex],
...
const getEscrowBalance = async (
  address: String
): Promise<EscrowBalanceInterface> => {
  try {
