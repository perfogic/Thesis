...
import {
  owalletAndroidDownloadUrl,
  owalletExtensionDownloadUrl,
  owalletIosDownloadUrl,
} from "pages/DownloadApp";
  ...
  return (
    <Modal
      ...
      className={classNames(
        styles.chooseWalletModalContainer,
        `${styles[theme]}`
      )}
    >
          ...
          <div className={styles.appStore}>
            <div onClick={() => window.open(owalletIosDownloadUrl)}>
              {theme === "light" ? (
                ...
                <AppstoreOWalletDarkIcon />
              )}
            </div>
            <div onClick={() => window.open(owalletAndroidDownloadUrl)}>
              {theme === "light" ? (
                <GooglePlayOWalletIcon />
              ) : (
                <GooglePlayOWalletDarkIcon />
              )}
            </div>
          </div>
          <div
            className={styles.chromeExt}
            onClick={() => window.open(owalletExtensionDownloadUrl)}
          >
            {theme === "light" ? (
              <ChromeExtOWalletIcon />
            ) : (
              <ChromeExtOWalletDarkIcon />
            )}
          </div>
        </div>
      </div>
